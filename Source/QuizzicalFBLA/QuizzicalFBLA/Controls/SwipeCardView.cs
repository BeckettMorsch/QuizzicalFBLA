...
namespace MLToolkit.Forms.SwipeCardView
{
    public class SwipeCardView : ContentView
    {
        ...
        private async void HandleTouchEnd()
        {
            ...
            if (position == DraggingCardPosition.FinishedOverThreshold)
            {
                bool supported = this.SupportedSwipeDirections.IsSupported(direction);
                ...
                if (supported)
                {
                    ...
                    else
                    {
                        await Task.WhenAll(new Task[] {
                            topCard.TranslateTo(0, _cardDistanceY > 0 ? this.Height : -this.Height, AnimationLength / 2, Easing.SpringOut),
                            topCard.FadeTo(0, AnimationLength / 6, Easing.Linear)
                            });
